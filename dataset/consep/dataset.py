...
def patch_path_to_name_convertor(patch_path):
    patch_name, _ = os.path.splitext(os.path.split(patch_path)[1])
    patch_name = patch_name.split('_')[-1]
#    print('**********************************', patch_name)
...
class CoNSePDataset(Dataset):
    ...
    def __init__(self, root_dir, transform=None, target_transform=None, patch_transform=None, patch_size=None,
        ...
        Args:
            ...
            transform (callable, optional): Optional transform to be applied
                on a sample.
        """
        super(CoNSePDataset, self).__init__()
    ...
    @property
    def data(self):
        return [self._read_item(i)[0] for i in range(self.__len__())]

    @property
    def targets(self):
        return [self._read_item(i)[1] for i in range(self.__len__())]

    @property
    def patches(self):
        return [self._read_item(i)[2] for i in range(self.__len__())]

    @property
    def segmentations(self):
        return [self._read_item(i)[6] for i in range(self.__len__())]

    @property
    def extra_features(self):
        return [self._read_item(i)[7] for i in range(self.__len__())]
    ...
    def _read_item(self, idx):
        ...
        location = [int(x) for x in open(self.location_files[idx], 'r').readline().strip('[]').split(' ') if
                    x.isdigit()]
#        location = [int(float(x.strip())) for x in open(self.location_files[idx], 'r').readline().split(',')]

        slide_id = patch_path_to_name_convertor(self.patch_files[idx])
...
#                inst_map = scipy.io.loadmat(self.segmentation_files[idx])['inst_map'].astype(int)
        ...
        if self.patch_size:
            ...
            if right - patch_left < self.patch_size:  # check for patch size
                patch_left = patch_left - (self.patch_size - right + patch_left)
            if bottom - patch_top < self.patch_size:  # check for patch size
                patch_top = patch_top - (self.patch_size - bottom + patch_top)
